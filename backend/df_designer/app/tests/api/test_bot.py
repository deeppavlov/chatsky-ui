...
from app.api.api_v1.endpoints.bot import (
    _stop_process, _check_process_status, start_build, check_build_processes,
    get_build_logs, start_run, check_run_processes, get_run_logs, connect
)
...
@pytest.mark.parametrize(
    "process_type, process_manager", [
        ("build", BuildManager), ("run", RunManager)
    ]
)
@pytest.mark.asyncio
...
@pytest.mark.parametrize(
    "error_type", [RuntimeError, ProcessLookupError]
)
@pytest.mark.asyncio
...
async def test_start_build(mocker):
    ...
    mocker.patch.multiple(
        preset, wait_time=0, end_status="loop"
    )
...
async def test_start_run(mocker):
    ...
    mocker.patch.multiple(
        run_manager,
        start=start,
        get_last_id=mocker.MagicMock(return_value=run_id),
        check_status=mocker.AsyncMock()
    )
    mocker.patch.multiple(
        preset, wait_time=0, end_status="loop"
    )

    response = await start_run(build_id=build_id, preset=preset, background_tasks=BackgroundTasks(), run_manager=run_manager)
