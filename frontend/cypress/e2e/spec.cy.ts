...
describe('check run and edit flow', () => {
  it('add run edit flow', () => {
    ...
    cy.request('http://localhost:8000/api/v1/flows').then((response) => {
      nodes = response.body.data.flows.find((flow) => flow.name === 'flow1')
        .data.nodes
      console.log(nodes)
    })
    ...
    cy.get('[data-testid=python-condition-editor]').type(
      'if False: print("hello")',
    )
    cy.get('[data-testid=save-condition-button]')
      .should('exist')
      .click({ force: true })
    cy.get('[data-testid=rf__wrapper]').trigger('keydown', {
      ctrlKey: true,
      key: 's',
    })
    cy.wait(500)
    ...
    cy.wrap(null).then(() => {
      let currNodes: any[] = []
      cy.request('http://localhost:8000/api/v1/flows').then((response) => {
        currNodes = response.body.data.flows.find(
