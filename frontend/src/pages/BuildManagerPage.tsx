...
import { SquareArrowOutUpRight, SquareIcon, X } from 'lucide-react'
  ...
  const handleConfirmRebuild = () => {
    openPopUp(
      <RebuildModal
        id='rebuild'
        onRebuild={async () => {
          const newBuildName = `Сборка ${builds.length}`
          const newRunName = `Запуск ${runs.length}`
          setLoading(true)
    ...
    <div className='absolute left-0 top-0 flex h-screen w-screen flex-col bg-background px-10 pb-12 pt-24 transition-all duration-300'>
      ...
      <div className='mb-4 flex w-full justify-between px-3 align-middle'>
        <div className='flex items-center gap-3'>
          <h2 className='text-2xl font-semibold'>Запуск</h2>
          <Button
            isDisabled={loading}
          ...
          >
            ...
            <span className='z-10 text-background'>Быстрый запуск</span>
          </Button>
        </div>
      </div>
      ...
      <div className='flex h-0 flex-grow flex-col gap-4 align-middle'>
        <div className='grid w-full flex-grow-0 grid-cols-5 gap-12 px-3'>
          <h3 className='text-lg font-semibold'>Новая сборка</h3>
          <h3 className='text-lg font-semibold'>Ваши сборки</h3>
          <h3 className='text-lg font-semibold'>Новый запуск</h3>
          <h3 className='text-lg font-semibold'>Запущено</h3>
          <h3 className='text-lg font-semibold'>Предыдущие запуски</h3>
        </div>
                          ...
                          <>
                            ...
                            >
                              <button
                                onClick={() => {
                                  navigate(
                                    `${location.pathname}?page=inspect&build_id=${b.id}&type=build`,
                                  )
                                }}
          ...
          <div className='relative h-full'>
            <div className='flex h-full flex-col gap-3'>
              ...
              >
                {aliveRuns.map((r) => {
                  ...
                  return (
                        ...
                        <div className='flex'>
                          ...
                          >
                            <button
                              ...
                              className='flex h-6 w-6 items-center justify-center fill-text-secondary stroke-none hover:fill-text active:scale-95'
                            >
                              <SquareIcon className='size-[18px] fill-inherit stroke-inherit' />
                            </button>
                          </Tooltip>
                          ...
                          >
                            <button
                              onClick={() => {
                                navigate(
                                  `${location.pathname}?page=inspect&run_id=${r.id}&type=run`,
                                )
                              }}
                              className='flex h-6 w-6 items-center justify-center hover:scale-105 active:scale-95'
                            >
                    ...
                    >
                      <StringItem content={['Сборка: ', r.preset.build_name]} />
                      <StringItem
              ...
              >
                ...
                <SquareIcon className='size-[18px] flex-shrink-0 fill-text stroke-none' />
