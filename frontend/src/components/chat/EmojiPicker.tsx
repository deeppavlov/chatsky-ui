...
const EmojiPicker = ({
  ...
  const recentlyHandler = (emoji: EmojiType) => {
    if (recently.some((e) => e.character === emoji.character)) {
      setRecently((prev) => [
        emoji,
        ...prev.filter((e) => e.character !== emoji.character),
      ])
    } else {
      ...
      setRecently((prev) =>
        [
          visibleData.find((e) => e.character === emoji.character),
          ...prev,
        ].slice(0, 10),
      )
    }
  ...
  useEffect(() => {
    if (observerElement_1.inView) {
      ...
      setVisibleData((prev) => [
        ...prev,
        ...data.slice(prev.length, prev.length + 40),
      ])
    }
        ...
        <div className='grid grid-cols-5 flex-wrap justify-evenly gap-0.5'>
          {recently?.map((emoji) => (
            <button
              ...
              className='hover:bg-accent flex h-[36px] w-[36px] items-center justify-center rounded-lg p-1 text-3xl transition'
            >
              {emoji.character}
            </button>
          ))}
