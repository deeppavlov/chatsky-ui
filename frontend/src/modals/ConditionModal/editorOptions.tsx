...
function twoLinesRange(view: EditorView, type: 'condition' | 'response') {
  ...
  const firstLineDecoration = createWidget(
    generateLineContent(type, 'first', firstLineName),
    0,
    firstLineText.length,
  )
  const secondLineDecoration = createWidget(
    generateLineContent(type, 'second', firstLineName),
    firstLineText.length + 1,
