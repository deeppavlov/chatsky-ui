...
const LayoutFlow = () => {
  ...
  useEffect(() => {
    setNodesLayout(nds.filter((node: NodeType) => node.data.node.base_classes[0] !== 'links').map((node: NodeType, index, nodes) => (
  ...
  useEffect(() => {
    ...
    setEdgesLayout(prev => [...prev, nds.filter((node: NodeType) => node.data.node?.base_classes[0] !== 'links').map((node: NodeType, idx, arr) => node.data.node.conditions?.filter((cond) => cond.transitionType !== 'default').sort((cond) => cond.transitionType === "repeat" ? -1 : 1).map((cond, index) => {
