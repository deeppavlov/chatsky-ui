...
const EmojiPicker = ({ data, lazy = false, theme = 'auto', onEmojiClick }: emojiPickerType) => {
  ...
  return (
    <div className="flex h-[244px] w-max flex-col items-center justify-start gap-2.5 overflow-y-scroll rounded-lg bg-background p-1 pt-1 scrollbar-hide">
      <div>
        ...
        <div className="grid grid-cols-5 flex-wrap justify-evenly gap-0.5 ">
          {recently?.map((emoji, index) => (
            <button
              onClick={(e) => {
                onEmojiClick(emoji.character, e)
