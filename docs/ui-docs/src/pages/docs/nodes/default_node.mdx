...
*Read more about prompt engineering in Chatsky UI.*
...
*Learn how to set up* messenger interfaces *in Chatsky UI.*
...
Every default node has to be connected to another one to work. Transitions work as such: a condition triggered in one node leads to a response in another node. To achieve this, select a condition in the first node that has to be triggered, find the orange circle in the right hand side and drag it to the blue circle in the second node’s response.
...
There also are transition functions that allow you create connections not dependant on physical links between two nodes, but on functions that analyse following nodes and previous ones, so that if you rearrange them or change completely, the pattern will stay the same.

Here is the list of all transition functions:

1. Forward - transitions to the next node in the queue
2. Backward - transitions to the ???
